    ...
    <div class="text-white">
      <h2
        :class="[
          ...
          battleResult === 'victory' ? 'text-semantic-success-500' : 'text-semantic-danger-500',
        ]"
      >
      ...
      <div class="mb-6 space-y-3">
        <div v-if="battleResult === 'victory'">
          ...
          <p v-if="battleType === 'countryside'" class="text-semantic-success-400">
        ...
        <div v-else>
          ...
          <div v-if="battleType === 'countryside'" class="text-semantic-danger-400">
            <p>Countryside infrastructure has been damaged.</p>
            <p class="text-sm mt-1">Your army retreats with remaining strength.</p>
          </div>
          <div v-else-if="battleType === 'wall'" class="text-semantic-danger-400">
      ...
      <div class="bg-semantic-neutral-700 rounded p-4 mb-6">
        ...
        <div class="space-y-2 text-sm">
          <div class="flex justify-between">
            <span class="text-semantic-neutral-400">Rounds:</span>
            <span class="font-semibold">{{ roundNumber }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-semantic-neutral-400">Remaining Army:</span>
            <span class="font-semibold">{{ armyCount }} units</span>
          </div>
          <div class="flex justify-between">
            <span class="text-semantic-neutral-400">Total Army Strength:</span>
      ...
      <!-- Battle Log Preview -->
      <div class="bg-semantic-neutral-800 rounded p-3 mb-6 max-h-40 overflow-y-auto">
        <div class="text-sm text-semantic-neutral-300 mb-2">Recent Rounds</div>
        <div class="space-y-1 text-xs">
          <div v-for="(log, index) in recentLogs" :key="index" class="text-semantic-neutral-200">
            {{ log }}
          </div>
        </div>
      </div>
      ...
      <button
        class="w-full bg-semantic-info-600 hover:bg-semantic-info-700 text-white font-semibold py-3 px-4 rounded transition-colors"
        @click="handleClose"
      >
        Continue
      </button>
