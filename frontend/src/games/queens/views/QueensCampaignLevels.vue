<template>
  ...
  >
    <div class="mx-auto min-h-dvh w-full max-w-[480px] px-5 pb-8 pt-5">
      ...
      <button
        type="button"
        class="rounded-xl border border-semantic-neutral-700 bg-semantic-neutral-900/80 px-3 py-2 text-sm font-semibold text-semantic-neutral-100 transition-colors hover:border-semantic-neutral-500 hover:bg-semantic-neutral-800"
      ...
      <div v-else class="pt-4">
        <div
          class="rounded-2xl border border-semantic-neutral-800 bg-semantic-neutral-900/55 px-4 py-3 text-sm text-semantic-neutral-300"
        >
        ...
        <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          >
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <div class="text-[1.65rem] font-bold leading-none text-white">Level {{ entry.levelNumber }}</div>
              ...
              <span
                ...
                class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-semantic-success-500/20 text-lg font-bold text-semantic-success-300"
                aria-hidden="true"
              >
                ✓
              </span>
            ...
            <div class="mt-auto space-y-2 pt-5 text-xs font-medium">
              <div
                ...
                class="inline-flex max-w-full rounded-full border border-semantic-warning-400/60 bg-semantic-warning-400/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-semantic-warning-200"
              >
                Current
              </div>

              <div class="text-sm text-semantic-neutral-300">
                <span v-if="entry.bestTime !== null">Best {{ queensStore.formatTime(entry.bestTime) }}</span>
...
import {
  useQueensStore,
  type QueensCampaignLevelEntry,
} from '@/games/queens/stores/queensStore';
...
function tileClass(entry: QueensCampaignLevelEntry): string {
  if (entry.isLocked) {
    return 'cursor-not-allowed border-semantic-neutral-800 bg-semantic-neutral-900/50 text-semantic-neutral-500 opacity-60';
  }

  if (entry.isCurrent) {
    return 'border-semantic-warning-400 bg-semantic-warning-500/12 shadow-[0_0_0_1px_rgba(251,191,36,0.18)] hover:border-semantic-warning-300 hover:bg-semantic-warning-500/16';
  }

  if (entry.isCompleted) {
    return 'border-semantic-success-700 bg-semantic-success-500/10 hover:border-semantic-success-500 hover:bg-semantic-success-500/14';
  }

  return 'border-semantic-neutral-700 bg-semantic-neutral-900/80 hover:border-semantic-info-500 hover:bg-semantic-neutral-800';
}
