    ...
    >
      ...
      <div class="flex flex-row gap-2 justify-center">
        ...
        >
          ...
          <span class="text-semantic-danger-300 font-semibold">–{{ tablePayouts.ant }} gold</span>
        </div>
        ...
        >
          ...
          <span class="text-semantic-success-300 font-semibold"
            >+{{ tablePayouts.honeypot }} gold</span
          >
        </div>
      </div>

      <!-- Max payout info -->
      <div v-if="tableStore.maxPayout > 0" class="mt-2 pt-2 border-t border-edge-plaqueGoldSoft">
        <div
          class="text-plaque-parchment text-opacity-90 text-shadow-[0_1px_0_rgba(0,0,0,0.35)] text-center mb-2"
        >
          Max you can win at this table:
          <span class="text-semantic-warning-300 font-semibold">{{ tableStore.maxPayout }}</span>
        </div>
