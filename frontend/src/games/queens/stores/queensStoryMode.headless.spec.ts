...
const RUN_FULL_STORY_SUITE = process.env.QUEENS_FULL_STORY === '1';
const fullStoryIt = RUN_FULL_STORY_SUITE ? it : it.skip;
...
describe('Queens story mode headless campaign', () => {
  ...
  async function createCampaignStore() {
  ...
  async function solveSingleLevel(
    levelIndex: number,
    options?: { expectFullCompletion?: boolean }
  ) {
    ...
    if (options?.expectFullCompletion) {
  ...
  fullStoryIt(
    'completes campaign levels 1-13 (tutorial + extra-easy) via hints',
    async () => {
      const { store, getOrderedApplicableQueensSolverSteps, campaignBuckets } =
        await createCampaignStore();
  ...
  fullStoryIt(
    'completes campaign levels 14-25 (easy + medium) via hints',
    async () => {
      const { store, getOrderedApplicableQueensSolverSteps, campaignBuckets } =
        await createCampaignStore();
  ...
  fullStoryIt(
    'completes campaign levels 26-33 (hard) via hints',
    async () => {
      const { store, getOrderedApplicableQueensSolverSteps, campaignBuckets } =
        await createCampaignStore();
  ...
  fullStoryIt(
    'completes campaign levels 34-37 (extra-hard) to full completion',
    async () => {
      const { store, getOrderedApplicableQueensSolverSteps, campaignBuckets } =
        await createCampaignStore();
