  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    ...
    >
      ...
      <div class="text-center mb-6">
        <h1 class="text-4xl font-bold text-semantic-warning-100 mb-2">Cassowary Casino</h1>
        <p class="text-semantic-warning-200">Choose your table and test your luck!</p>
      ...
      <div class="flex justify-center mb-4">
        <div class="flex items-center space-x-4">
          <label class="flex items-center space-x-3 text-semantic-warning-200 cursor-pointer">
            <div class="relative">
              <input v-model="hideUnavailableTables" type="checkbox" class="sr-only" />
              <div
                class="w-12 h-6 bg-semantic-warning-900 border-2 border-semantic-warning-600 rounded-full transition-colors duration-200"
                :class="
                  hideUnavailableTables ? 'bg-semantic-warning-600 border-semantic-warning-500' : ''
                "
              ></div>
              <div
                class="absolute top-0.5 left-0.5 w-5 h-5 bg-semantic-warning-300 rounded-full transition-transform duration-200 shadow-md"
                :class="hideUnavailableTables ? 'transform translate-x-6' : ''"
              ></div>
            </div>
            <span class="text-sm font-medium">Hide unavailable tables</span>
          </label>
          <span class="text-semantic-warning-300 text-sm">
      ...
      <div v-if="!tableStore.loaded" class="text-center py-12">
        <div class="text-semantic-warning-200 text-xl">Loading tables...</div>
      </div>
      ...
      <div v-else>
        ...
        <div v-if="filteredTables.length === 0" class="text-center py-12">
          <div class="text-semantic-warning-200 text-xl mb-4">
            {{ hideUnavailableTables ? 'No available tables found' : 'No tables available' }}
          </div>
          <div v-if="hideUnavailableTables" class="text-semantic-warning-300 text-sm">
          ...
          >
            <div class="text-center">
              <div class="flex items-center justify-center gap-2 mb-2">
                <h3 class="text-xl font-bold text-semantic-warning-100">{{ table.name }}</h3>
              </div>
              <div class="space-y-2 text-semantic-warning-200 text-sm">
                <div class="flex justify-between">
                  <span>Min Balance:</span>
                  <span class="font-semibold text-semantic-warning-300"
                    >{{ table.minimumBuyIn }} chips</span
                  >
                </div>
                <div class="flex justify-between">
                  <span>Max Payout:</span>
                  <span class="font-semibold text-semantic-warning-300"
                    >{{ table.maxPayout }} chips</span
                  >
                </div>
                <div class="flex justify-between">
                  <span>Board Size:</span>
                  <span class="font-semibold text-semantic-warning-300">{{ table.boardSize }}</span>
                </div>
                <div class="flex justify-between">
                  <span>Multiplier:</span>
                  <span class="font-semibold text-semantic-info-300"
                    >{{ table.payoutMultiplier }}x</span
                  >
                </div>
                <div class="flex justify-between">
              ...
              <div class="mt-4">
                <button
                  ...
                  :class="
                    globalStore.isTableAccessible(table.id, table.minimumBuyIn, table.maxPayout)
                      ? 'bg-gradient-to-r from-semantic-warning-500 to-semantic-warning-400 text-semantic-warning-900 hover:from-semantic-warning-400 hover:to-semantic-warning-300 shadow-lg hover:shadow-xl'
              ...
              <!-- Progress indicator -->
              <div
                v-if="globalStore.tablesProgress[table.id]"
              ...
              >
                    ...
                    :class="
                      globalStore.tablesProgress[table.id].totalProfit < 0
                        ...
                        : globalStore.tablesProgress[table.id].totalProfit >= table.maxPayout
                          ...
                          : 'text-semantic-warning-300'
                    "
                    >{{ globalStore.tablesProgress[table.id].totalProfit }} /
                    {{ table.maxPayout }}</span
                  >
                </div>
                <div class="w-full bg-semantic-warning-900 rounded-full h-1">
                  <div
                    :class="
                      globalStore.tablesProgress[table.id].totalProfit >= table.maxPayout
                        ? 'bg-semantic-success-400'
                        : 'bg-semantic-warning-400'
                    "
                ...
                <div
                  ...
                  class="mt-2 text-center text-semantic-success-300 font-semibold text-xs"
                >
                  ✓ Table Complete
                </div>
