<template>
  <div class="w-full">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
      <!-- Wall Zone Column -->
      <div class="bg-gray-800/50 rounded-lg p-4 sm:p-6 border border-gray-600 min-w-0">
        ...
        <div class="mb-3 pb-3 border-b border-gray-600">
          <div class="text-xs sm:text-sm text-gray-400 mb-1">Defense Bonus</div>
          <div class="text-lg sm:text-xl font-bold text-yellow-400">
            +{{ wallDefenseBonus }}
            <span v-if="wallDivineMultiplier > 1" class="text-xs sm:text-sm text-yellow-500 ml-1">
              (×{{ wallDivineMultiplier.toFixed(2) }})
            </span>
          </div>
        </div>
        <div class="space-y-2 min-h-[100px]">
          <div v-if="wallCards.length === 0" class="text-xs sm:text-sm text-gray-500 italic py-3">
            No wall upgrades
          </div>
          <div
            v-for="card in wallCards"
            :key="card.id"
            class="bg-gray-700/50 rounded p-2 border border-gray-600 min-w-0"
          >
            <div class="text-xs sm:text-sm font-semibold text-white break-words" :title="card.name">
              {{ card.name }}
            </div>
            <div class="text-xs text-yellow-400 mt-1">🛡️ +2</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-gray-800/50 rounded-lg p-4 sm:p-6 border border-gray-600 min-w-0">
        ...
        <div class="mb-3 pb-3 border-b border-gray-600">
          <div class="text-xs sm:text-sm text-gray-400 mb-1">Income</div>
          <div class="text-lg sm:text-xl font-bold text-green-400">
            🪙 {{ cityIncome }}
            <span v-if="cityDivineMultiplier > 1" class="text-xs sm:text-sm text-yellow-500 ml-1">
              (×{{ cityDivineMultiplier.toFixed(2) }})
            </span>
          </div>
        </div>
        <div class="space-y-2 min-h-[100px]">
          <div v-if="cityCards.length === 0" class="text-xs sm:text-sm text-gray-500 italic py-3">
            No city buildings
          </div>
          <div
          ...
          >
            ...
            <div class="text-xs text-green-400 mt-1">🪙 +{{ card.revenue }}/turn</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-gray-800/50 rounded-lg p-4 sm:p-6 border border-gray-600 min-w-0">
        ...
        <div class="mb-3 pb-3 border-b border-gray-600">
          ...
          <div class="text-lg sm:text-xl font-bold text-green-400">
            ...
            <span
              ...
              class="text-xs sm:text-sm text-yellow-500 ml-1"
            >
        ...
        <div class="space-y-2 min-h-[100px]">
          <div
            ...
            class="text-xs sm:text-sm text-gray-500 italic py-3"
          >
          ...
          >
            ...
            <div v-if="card.damaged" class="text-xs text-red-400 mt-1">Damaged</div>
            <div v-else class="text-xs text-green-400 mt-1">🪙 +{{ card.revenue }}/turn</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-gray-800/50 rounded-lg px-4 py-2 border border-gray-600">
      <div class="flex items-center justify-center gap-2 sm:gap-4 text-xs sm:text-sm flex-wrap">
        <span class="text-gray-400 whitespace-nowrap">Income this year:</span>
        <span class="font-semibold text-green-400 whitespace-nowrap">City 🪙 {{ cityIncome }}</span>
        <span class="text-gray-600 hidden sm:inline">|</span>
        <span class="font-semibold text-green-400 whitespace-nowrap"
