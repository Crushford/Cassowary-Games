...
export function ratePuzzleAttributes(layout: string): PuzzleAttributeRatings {
  ...
  const elongationMean =
    elongationValues.reduce((sum, value) => sum + value, 0) / Math.max(elongationValues.length, 1);
...
export function differenceByAttribute(
  ...
  return {
    ...
    regionAdjacencyDensity: Math.abs(left.regionAdjacencyDensity - right.regionAdjacencyDensity),
  };
}
...
export function pickBestInsertionIndexForNewFamily(
  ...
  const ratingsByFamily = new Map<string, PuzzleAttributeRatings>(
    existingFamiliesInOrder.map((family) => [
      family.familyId,
      ratePuzzleAttributes(family.primary.layout),
    ])
  );
