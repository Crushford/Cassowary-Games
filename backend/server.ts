...
app.get('/api/puzzles/random/:size', (req, res) => {
  try {
    ...
    if (validPuzzles.length === 0) {
      return res
        .status(404)
        .json({ error: `No valid ${size}x${size} puzzles found` })
    }
...
app.post('/api/puzzles/save', (req, res) => {
  try {
    ...
    if (!isValidPuzzle(layout, queens, size)) {
      return res.status(400).json({
        error:
          'Invalid puzzle: must have exactly gridSize queens and sufficient color groups'
      })
    }
...
function convertStringToGrid(layout: string, queens: string, gridSize: number) {
  ...
  const SYMBOL_TO_COLOR: Record<string, string> = Object.entries(
    COLOR_SYMBOLS
  ).reduce((acc, [color, symbol]) => {
    if (color !== 'undefined') {
      acc[symbol] = color
    }
    return acc
