...
function findDuplicateColorGroups(
...
function main(): void {
  ...
  for (const [sizeKey, puzzles] of Object.entries(data)) {
    ...
    const familiesToRemove = new Set(
      groups.flatMap(group => group.removeFamilies)
    )
    ...
    for (const group of groups) {
      console.log(
        `  keep ${group.keepFamily}, remove ${group.removeFamilies.join(', ')}`
      )
    }
