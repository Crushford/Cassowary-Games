...
function reorderOneSize(
  ...
  const finalOrder =
    reorderedContrast >= originalContrast
      ? reorderedFamilies
      : families

  const changed = finalOrder.map(f => f.familyId).join('|') !== families.map(f => f.familyId).join('|')
  ...
  console.log(`  adjacent contrast after:  ${Math.max(originalContrast, reorderedContrast).toFixed(2)}`)
