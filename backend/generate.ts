 ...
 */
function validateUniqueQueenColors(
  grid: GridSquare[][],
  ...
  if (colors.size !== expectedCount) {
    throw new Error(
      `Expected ${expectedCount} unique queen colors, got ${colors.size}`
    )
  }
}
...
const isSolvable = (state: GeneratorState): boolean => {
  ...
  const isSolved = state.autoTestMarks.every(r =>
    r.every(c => c === 'flag' || c === 'queen')
  )
...
function initializeGrid(size: number = SIZE): GeneratorState {
  ...
  const autoTestMarks: MarkType[][] = Array.from({ length: size }, () =>
  ...
  if (square.groupColor) {
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        if (
          marks[r][c] === 'queen' &&
          grid[r][c].groupColor === square.groupColor
        ) {
          return false
        }
      }
...
function placeAllQueens(state: GeneratorState, size: number): void {
  ...
  const tempQueenMarks: MarkType[][] = Array.from({ length: size }, () =>
    Array(size).fill(null)
  )
  let attempts = 0
  ...
  if (finalQueenCount === size) {
    ...
    throw new Error(
      `Failed to place all ${size} queens after ${maxAttempts} attempts`
    )
  }
}
...
function expandColorGroupsInPlace(grid: GridSquare[][]): void {
  ...
  const byColor: Record<
    string,
    Array<{ row: number; col: number; square: GridSquare }>
  > = {}
...
function placeQueen(state: GeneratorState, row: number, col: number): void {
  ...
  if (!state.grid[row][col].isSolutionQueen) {
    ...
    throw new Error(
      `Attempted to place queen at (${row}, ${col}) but it's not a solution queen`
    )
  }
...
function flagBlockingSquares(state: GeneratorState) {
  ...
  forEveryCell(size, (row, col) => {
    ...
    const colFullyBlocked = isLineFullyBlocked(c =>
      simulatedMarks.map(r => r[c])
    )
    const colorGroups = new Map()
    forEveryCell(size, (r, c) => {
  ...
  for (const [axisKey, allowedColors] of axisSetToColors.entries()) {
    ...
    for (const primaryIndex of axisValues) {
      for (let secondaryIndex = 0; secondaryIndex < size; secondaryIndex++) {
        ...
        const isOutsideAllowedColors =
          !squareColor || !allowedColors.has(squareColor)
...
function dumpMarks(marks: MarkType[][]) {
  ...
  console.log(
    marks
      .map(row =>
        row
          .map(cell => (cell === 'queen' ? 'Q' : cell === 'flag' ? 'F' : '.'))
          .join(' ')
      )
      .join('\n')
  )
